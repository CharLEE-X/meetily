...
function MessageBubble({
  ...
  onCitationSelect: (citationKey: string, citation: MeetingChatCitation) => void;
  onRetry?: () => void;
}) {
      ...
      <div className={`min-w-0 max-w-[82%] rounded-xl border px-4 py-3 shadow-sm ${isUser
        ...
        {isUser ? (
          ...
          <ReactMarkdown
            ...
            className="prose prose-sm max-w-none break-words text-slate-900 prose-p:my-2 prose-p:leading-6 prose-ul:my-2 prose-ol:my-2 prose-li:my-1 prose-strong:text-slate-950 prose-a:text-emerald-700 prose-a:no-underline hover:prose-a:underline"
            components={{
              a: ({ href, children }) => (
            ...
            {message.citations.map((citation) => {
              ...
              return (
                <CitationChip
                  ...
                  onSelect={() => onCitationSelect(citationKey, citation)}
                />
              );
            })}
          </div>
...
export default function SummaryChatPage() {
  ...
  useEffect(() => {
    ...
    void listenToMeetingChatStream((event) => {
      ...
      if (event.kind === 'started') {
        const previousPendingId = pendingAssistantIdRef.current;
        pendingAssistantIdRef.current = event.messageId;
        setMessages((current) => current.map((message) => (
          previousPendingId && message.id === previousPendingId
            ? {
              ...message,
              id: event.messageId,
              content: '',
      ...
      if (event.kind === 'delta' && event.delta) {
        const targetId = pendingAssistantIdRef.current ?? event.messageId;
        setMessages((current) => current.map((message) => (
          message.id === targetId || message.id === event.messageId
            ? {
              ...message,
              id: event.messageId,
  ...
  const askQuestion = async (value: string) => {
    ...
    pendingAssistantIdRef.current = optimisticAssistant.id;
